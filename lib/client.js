...
try {
    ...
    console.log("🌍 Connected to the VAJIRA  DB")
} catch {
...
let cc = Config.sessionName.replace(/VAJIRA;;;/g, "").replace(/VAJIRA;;;/gi, ""); // DONT REMOVE BLACK;;; so you can connect BLACK md session id too
 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    ...
                    console.log("⬇️  Installing VAJIRA External Plugins...");
                    ...
                    console.log("😁  Vajira Plugins Installed!");fs.readdirSync(__dirname + "/../commands").forEach((plugin) => {
